...
export function StoreItemCard({ store, isSelected, onClick }: StoreItemCardProps) {
    ...
    const logoSrc = getImg(store.logo?.[0], undefined, store.url).match(/url\("?([^"]+)"?\)/)?.[1];
    const coverSrc = getImg(store.cover_image?.[0], 'cover', store.url).match(/url\("?([^"]+)"?\)/)?.[1];
    ...
    return (
        ...
        <button // Utiliser bouton pour accessibilité
            type="button"
            onClick={onClick}
            className={`store-item-card w-full h-full flex flex-col rounded-xl overflow-hidden shadow-md border-2 transition-all duration-200 ease-in-out transform hover:-translate-y-1 cursor-pointer focus:outline-none ${
                isSelected ? 'border-blue-500 shadow-blue-300/50 scale-100' : 'border-gray-200 hover:border-gray-300 hover:shadow-lg'
            }`}
        >
            {/* Image de Couverture */}
            <div
                className="w-full h-24 sm:h-28 flex-shrink-0 bg-cover bg-center bg-gray-200 relative"
                style={{ backgroundImage: coverSrc && !coverError ? `url(${coverSrc})` : `url(${NO_PICTURE})` }} // Style inline pour bg
