...
export function StoresList({isFilterActive,maxVisible=5.8, maxSize=1200,stores, isLoading, selectedStoreId, onSelectStore, viewAllUrl, newStoreRequire }: StoresListProps) {
    ...
    const onManage = (store:StoreInterface)=>{
        ...
        // Utiliser navigate de Vike pour une navigation plus fluide
        navigate('/store')
    }
        ...
        return (
            <div className='fixed inset-0 z-[9999] flex items-center justify-center bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm'>
                <div className="text-center p-8">
                    ...
                    <p className="text-slate-600 dark:text-slate-300 text-lg font-medium">Chargement des informations...</p>
                </div>
            </div>
        )
    }
