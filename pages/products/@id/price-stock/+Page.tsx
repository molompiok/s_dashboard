...
import { useGetProducts, useMultipleUpdateFeaturesValues } from '../../../../api/ReactSublymusApi'; // ✅ Hooks API
...
function Page() {
    ...
    const saveChanges = async () => {
        const featuresToUpdate = features.filter(f => (f as any)[EDITED_DATA] === EDITED_DATA);
        if (featuresToUpdate.length === 0 || !product?.id) {
            logger.info("No feature changes to save.");
            return; // Rien à sauvegarder
        }

         // Préparer les données pour la mutation multiple (seulement updates de values ici)
         const updatePayload: {
             values: Record<string, { update_values: Partial<ValueInterface>[] }>
             create_features?: []; delete_features_id?: []; update_features?: []; // Non utilisés ici
         } = { values: {} };

         featuresToUpdate.forEach(f => {
            if (!f.id) return; // Ne devrait pas arriver
            const updatedValues = f.values?.filter(v => (v as any)[EDITED_DATA] === EDITED_DATA);
            if (updatedValues && updatedValues.length > 0) {
                if (!updatePayload.values[f.id]) {
                    updatePayload.values[f.id] = { update_values: [] };
                }
                 updatePayload.values[f.id].update_values.push(...updatedValues.map(v => ({...v}))); // Cloner les valeurs
            }
         });


         if (Object.keys(updatePayload.values).length === 0) {
             logger.info("No value changes detected in modified features.");
              // Reset les flags EDITED_DATA ?
              setFeatures(prev => prev.map(f => ({ ...f, [EDITED_DATA]: undefined, values: f.values?.map(v => ({...v, [EDITED_DATA]: undefined})) })));
             return;
         }

         const formData = new FormData();
         formData.append('product_id', product.id);
         formData.append('multiple_update_features', JSON.stringify(updatePayload));
         // Pas de fichiers à ajouter ici car on ne modifie que prix/stock

         multipleUpdateMutation.mutate(formData, {
                                   ...
                                   <div className="bind-feature-values flex flex-wrap gap-2">
                                       {f.values?.map(v => (
                                           ...
                                           <button
                                               ...
                                               className={`checkable inline-flex items-center gap-1.5 p-1 border rounded-md cursor-pointer transition hover:bg-gray-50 ${
                                                   (filter[f.id!]?.includes(v.id)) ? 'border-blue-400 bg-blue-50' : 'border-gray-300'
                                               }`}
                                               onClick={() => handleFilterToggle(f.id!, v.id)}
                                           >
                                                {/* Checkbox visuelle */}
                                                <span className={`check w-5 h-5 flex items-center justify-center rounded border transition ${
                                                     (filter[f.id!]?.includes(v.id)) ? 'bg-blue-600 border-blue-600 text-white' : 'border-gray-400 text-transparent'
                                                }`}>
