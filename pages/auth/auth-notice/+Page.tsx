...
import { useResendVerificationEmail } from '../../../api/ReactSublymusApi';
import { SpinnerIcon } from '../../../Components/Confirm/Spinner';
import { showToast } from '../../../Components/Utils/toastNotifications';
...
function Page() {
            ...
            onClick() {
              resendMutation.mutate({ email },{
                onSettled(){
                  ...
                  setTimeout(() => {
                    setIsLoading(false)
                    showToast("L'e-mail de vérification a bien été envoyé");
                  }, 1500);
                }
              })
            }, textKey: 'authNotice.action.resendVerification'
          } : undefined,
        };
      case 'email_verified':
        return {
  ...
  return (
    <div className={`absolute pb-[200px]  w-full h-full top-0 left-0 min-h-screen flex flex-col items-center justify-center bg-gradient-to-br ${gradientBackground} px-4 py-12 overflow-hidden`}>
        ...
        <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md p-8 sm:p-10 md:p-12 rounded-xl shadow-2xl border border-slate-200/50 dark:border-slate-700/50">
          ...
          <div className="mt-8 space-y-4 sm:space-y-0 sm:flex sm:flex-col sm:items-center sm:gap-4">
            ...
            {noticeContent.secondaryAction && (
              noticeContent.secondaryAction.onClick ?
                <div id='secondary-action' onClick={noticeContent.secondaryAction.onClick}
                  ...
                  {isLoading ? (
                    <>
                      <SpinnerIcon/>
                      <span className='ml-8'>Envoi...</span> {/* Ou laisse vide */}
                    </>
                  ) : (
                    t(noticeContent.secondaryAction.textKey)
                  )}
                </div> :
                noticeContent.secondaryAction.href && <Link
