...
function Page() {
    ...
    const saveThemeSettingsMutation = { isPending: false, mutate: (data: any, options: any) => { console.log("Saving settings:", data); setTimeout(() => { if (Math.random() > 0.1) options.onSuccess?.(data.settings); else options.onError?.(new ApiError("Save failed simulation", 500)); }, 1000); } };
    ...
    // --- Handler pour la sauvegarde (INCHANGÉ) ---
    const handleSaveSettings = () => {
        if (!hasChanges || saveThemeSettingsMutation.isPending || !storeId || !themeId) return;
        saveThemeSettingsMutation.mutate( { store_id: storeId, theme_id: themeId, settings: draftSettings }, { onSuccess: () => { logger.info("Theme settings saved successfully"); setSavedSettings(draftSettings); setHasChanges(false); }, onError: (error: ApiError) => { logger.error({ error }, "Failed to save theme settings"); } } );
    };
        ...
        <div className="w-full h-screen flex flex-col bg-gray-100">
            ...
            <div className="relative flex flex-row flex-grow overflow-hidden">
                ...
                <aside className={`
                    ...
                    lg:relative lg:inset-auto lg:z-auto lg:w-80 xl:w-96 /* Overrides Desktop */
            ...
            {/* Bouton Enregistrer Flottant (INCHANGÉ) */}
            <div className={`fixed bottom-4 right-4 z-50 transition-opacity duration-300 ${hasChanges ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                <SaveButton loading={saveThemeSettingsMutation.isPending} required={hasChanges} onClick={handleSaveSettings} title={t('themeEditor.saveButton')} effect="color" />
            </div>
        </div>
    );
}